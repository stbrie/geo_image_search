...
try:
    opts, args = getopt.getopt(sys.argv[1:],
                                "hofiva:", ["address=","output-directory","find-only","image-addresses","verbose"])
    print ('OPTIONS       :',opts)
...
jpeg_file_regex = re.compile('^.*\.(jpg)|(jpeg)$')

files_list = []
file_counter = 0
for dirpath, dirnames, filenames in walk(root_images_directory)
    if verbose:
        print (dirpath)
            ...
            with open(imagename,'rb') as image_file:
                ...
                if lat_deg_dec and long_deg_dec:
                    ...
                    if distance.distance(search_loc, image_loc).miles < .5:
                        if verbose:
                            print ("+ " + loc_format.format(file_name, lat_deg_dec, long_deg_dec, distance.distance(search_loc, image_loc).miles))
                        if output_directory:
